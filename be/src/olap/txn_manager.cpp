...
OLAPStatus TxnManager::commit_txn(OlapMeta* meta, TPartitionId partition_id,
    ...
    {
        // get tx
        std::shared_lock<std::shared_mutex> rdlock(_get_txn_map_lock(transaction_id));
        txn_tablet_map_t& txn_tablet_map = _get_txn_tablet_map(transaction_id);
        auto it = txn_tablet_map.find(key);
        if (it != txn_tablet_map.end()) {
            auto load_itr = it->second.find(tablet_info);
...
OLAPStatus TxnManager::publish_txn(OlapMeta* meta, TPartitionId partition_id,
    ...
    std::unique_lock<std::mutex> txn_lock(_get_txn_lock(transaction_id));
    {
        std::shared_lock<std::shared_mutex> rlock(_get_txn_map_lock(transaction_id));
        txn_tablet_map_t& txn_tablet_map = _get_txn_tablet_map(transaction_id);
        auto it = txn_tablet_map.find(key);
        if (it != txn_tablet_map.end()) {
            auto load_itr = it->second.find(tablet_info);
...
void TxnManager::get_tablet_related_txns(TTabletId tablet_id, SchemaHash schema_hash,
    ...
    for (int32_t i = 0; i < _txn_map_shard_size; i++) {
        std::shared_lock<std::shared_mutex> txn_rdlock(_txn_map_locks[i]);
...
void TxnManager::get_txn_related_tablets(const TTransactionId transaction_id,
    ...
    pair<int64_t, int64_t> key(partition_id, transaction_id);
    std::shared_lock<std::shared_mutex> txn_rdlock(_get_txn_map_lock(transaction_id));
    txn_tablet_map_t& txn_tablet_map = _get_txn_tablet_map(transaction_id);
    auto it = txn_tablet_map.find(key);
    if (it == txn_tablet_map.end()) {
