...
OLAPStatus PushHandler::_do_streaming_ingestion(TabletSharedPtr tablet, const TPushReq& request,
    ...
    std::shared_lock<std::shared_mutex> base_migration_rlock(tablet->get_migration_lock(),
                                                             std::try_to_lock);
    ...
    if (push_type == PUSH_FOR_DELETE) {
        for (TabletVars& tablet_var : *tablet_vars) {
            ...
            {
                std::shared_lock<std::shared_mutex> rdlock(tablet_var.tablet->get_header_lock());
